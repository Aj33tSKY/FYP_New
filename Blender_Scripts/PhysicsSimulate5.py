...
output_folder1 = r"C:\Ajeet\3rdYearProject\JUPYTER\10B-2D-Uni_OG\Stable" #Stable Class
output_folder2 = r"C:\Ajeet\3rdYearProject\JUPYTER\10B-2D-Uni_OG\Unstable" #Unstable Class
...
def setup_scene():
    ...
    ground.location = (0, 0, 0)  # Slightly below the blocks
            ...
            if i == 0:
                ...
                if remaining_left > 0.5 and random.random() < 0.75:
                    if total_blocks < num_blocks:
                        ...
                        total_blocks += 1
                        new_max_offset = remaining_left * 0.6
...
def render_scene(iteration, stability_status):
    ...
    if camera and camera.type == 'CAMERA':
        ...
        camera.location.z = min_coords.z + (total_height * 0.5)  # Adjust height to half of the max height
        
        # Lock the camera's X-axis rotation at 90 degrees
        camera.rotation_euler[0] = 1.5708  # 90 degrees in radians
        camera.rotation_euler[1] = 0
        camera.rotation_euler[2] = 0  # Keep Z-axis rotation as needed

        # Adjust the orthographic scale based on the average of total height and width
        camera.data.ortho_scale = (total_height + total_width) * 1.1 # Average divided by 4 to fit everything in view
